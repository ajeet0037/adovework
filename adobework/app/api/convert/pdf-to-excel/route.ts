...
/**
 * PDF to Excel API Route
 * Proxies to Python FastAPI backend for table extraction
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${PYTHON_BACKEND_URL}/api/v1/pdf/to-excel`, {
      method: 'POST',
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: error.error || error.detail || 'Failed to convert PDF to Excel' },
    ...
    return NextResponse.json(
      { error: 'Failed to convert PDF to Excel. Please try again.' },
      { status: 500 }
    );
  }
}
