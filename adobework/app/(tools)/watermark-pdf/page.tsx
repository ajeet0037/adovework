...
export default function WatermarkPdfPage() {
  ...
  const handleProcess = useCallback(async (): Promise<ProcessingResult> => {
    ...
    if (!watermarkText.trim()) {
      return { success: false, error: 'Please enter watermark text' };
    }
    ...
    try {
      ...
      const watermarkedPdfBytes = await addTextWatermark(buffer, {
        text: watermarkText,
        fontSize,
        opacity,
        rotation,
      ...
      const newBuffer = new ArrayBuffer(watermarkedPdfBytes.byteLength);
      new Uint8Array(newBuffer).set(watermarkedPdfBytes);
      ...
      const errorMessage = error instanceof Error ? error.message : 'Watermark failed';
  ...
  }, [files, watermarkText, fontSize, opacity, rotation, position, updateFileStatus, updateFileProgress]);
  ...
  const contentSections = [
    {
      heading: 'About PDF Watermarking',
      content: `
        <p>PDF watermarking adds text or image overlays to your documents for branding or security. 
        AdobeWork processes files directly in your browser for maximum privacy.</p>
        <p>Customize the watermark text, size, opacity, rotation, and position to suit your needs.</p>
      `,
    },
    {
      heading: 'How to Add Watermarks',
      content: `
        <ol>
          <li><strong>Upload</strong> - Select your PDF file.</li>
          <li><strong>Customize</strong> - Set watermark text and appearance.</li>
          <li><strong>Apply</strong> - Click Apply to add the watermark.</li>
          <li><strong>Download</strong> - Download your watermarked PDF.</li>
        </ol>
      `,
    },
  ];
  ...
  const file = files[0];
  const canWatermark = hasFile && !isProcessing && watermarkText.trim() && file?.status !== 'completed';
            ...
            <div className="flex flex-wrap gap-3">
              ...
              {!isProcessing && (
                <Button variant="outline" size="lg" onClick={handleClearFiles}>
                  {result?.success ? 'Watermark Another' : 'Clear'}
                </Button>
              )}
            </div>
